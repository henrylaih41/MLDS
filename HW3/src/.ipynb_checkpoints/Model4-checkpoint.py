    ...
    def build_model(self,z,real_image):
        ...
        def build_discriminator(image,reuse=False):
            ...
            with tf.variable_scope("discriminator") as scope:
                ...
                print(f4.shape)
                #f5 = ut.conv2d(f4,f4.shape[-1]*2,name="conv5")

                ### DNN layer
                flatten = tf.reshape(f4,[self.batch_size,-1])
                print(flatten.shape)
                #W1 = tf.get_variable("W1",[flatten.shape[-1],flatten.shape[-1]],tf.float32,
                #                     tf.truncated_normal_initializer(stddev=0.02))
                #B1 = tf.get_variable("B1",[flatten.shape[-1]],tf.float32,
                #                     tf.truncated_normal_initializer(stddev=0.02))
                W2 = tf.get_variable("W2",[flatten.shape[-1],1],tf.float32,
                                     tf.truncated_normal_initializer(stddev=0.02))
                B2 = tf.get_variable("B2",[1],tf.float32,
                                     tf.truncated_normal_initializer(stddev=0.02))
                #s1    = tf.matmul(flatten,W1) + B1
...
parser.add_argument('--save_path', type=str, default="../models3/")
parser.add_argument('--image_save_path', type=str, default="default/")
...
parser.add_argument('--nor',type=int, default=1)
