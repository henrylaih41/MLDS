...
class DatasetTrain(DatasetBase):
    ...
    def dump_tokenizer(self):
        ...
        with open('others/word_index.pkl', 'wb') as handle:
            pickle.dump(self.word_index, handle)
        with open('others/idx_to_word.pkl', 'wb') as handle:
            pickle.dump(self.idx_to_word, handle)
        with open('others/word_counts.pkl', 'wb') as handle:
    ...
    def schedule_sampling(self, epoch, cap_len_batch):
        sampling_prob = min( 0.2* (epoch/5),0.7)
