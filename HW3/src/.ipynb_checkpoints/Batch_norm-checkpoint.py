...
import cv2
class DCGAN():
    def __init__(self, args, noise_dim=100, batch_size=256, lr=0.0005, beta1=0.5):
    ...
    def build_model(self,z,real_image):
        def build_generator():
            ...
            with tf.variable_scope("generator") as scope:
                ...
                deconv1 = ut.deconv2d(DNN_output,[bs,d_w*2,d_h*2,int(depth/2)],name="deconv1",batch_norm=True)
                print(deconv1.shape)
                bs, d_w, d_h, depth = deconv1.get_shape().as_list()
                deconv2 = ut.deconv2d(deconv1,[bs,d_w*2,d_h*2,int(depth/2)],name="deconv2",batch_norm=True)
                print(deconv2.shape)
                bs, d_w, d_h, depth = deconv2.get_shape().as_list()
                deconv3 = ut.deconv2d(deconv2,[bs,d_w*2,d_h*2,int(depth/2)],name="deconv3",batch_norm=True)
                print(deconv3.shape)
        ...
        def build_discriminator(image,reuse=False):
            ...
            with tf.variable_scope("discriminator") as scope:
                ...
                f1 = ut.conv2d(image,conv_dim,f_h=7,f_w=7,name="conv1",batch_norm=True)
                print(f1.shape)
                f2 = ut.conv2d(f1,f1.shape[-1]*2,name="conv2",batch_norm=True)
                print(f2.shape)
                f3 = ut.conv2d(f2,f2.shape[-1]*2,name="conv3",batch_norm=True)
                print(f3.shape)
                f4 = ut.conv2d(f3,f3.shape[-1]*2,name="conv4",batch_norm=True)
                print(f4.shape)
            ...
            for epo in range(0, epoch):
                if (epo % 1 == 0):
                    ...
                    for i in range(self.genImage_num):
                        if(self.cv2):
                            cv2.imwrite("../genImage/" + self.image_save_path + self.name + "_" +                                               str(epo) + "_" + str(i) + ".jpg", gen_image[i])
                        else:
...
parser.add_argument('--name', type=str, default="DCGAN_default")
parser.add_argument('--save_path', type=str, default="../models_norm/")
parser.add_argument('--image_save_path', type=str, default="/norm/")
