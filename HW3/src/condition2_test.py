...
from dataProcessv3 import Testdataset
import skimage
import argparse
import cv2
import matplotlib.pyplot as plt
plt.switch_backend('agg')
load_path = "../Cond_Models/DCGAN_conditional_.ckpt-57"
read_csv = True
class DCGAN():
    def __init__(self, args, noise_dim=100, batch_size=25, lr=0.00003, beta1=0.5):
        self.d_rate = 1.2
    ...
    def test(self):
        test_batch_size = 25
        noise       = tf.placeholder(tf.float32,[test_batch_size,self.noise_dim+self.word_dim])
        real_image  = tf.placeholder(tf.float32,[test_batch_size,self.image_size,
                                                 self.image_size,self.image_depth])
        real_tags   = tf.placeholder(tf.float32,[test_batch_size,self.word_dim])
        random_tags = tf.placeholder(tf.float32,[test_batch_size,self.word_dim])
        self.build_model(noise,real_image,real_tags,random_tags)
        
        
        test_label_csv ="./tags.csv"
        ...
        with tf.Session() as sess:
            ...
            print("Saver Loaded: " + load_path)
            
            if read_csv:
                all_image = []
                total_num = 10
                for f in range(1):
                    datasetTest = Testdataset(test_label_csv, self.save_dict, test_batch_size)
                    batch_z   = ut.genNoise(test_batch_size,mode=self.noise_mode,ball=self.ball)
                    word_test = datasetTest.next_batch()
                    ...
                    if(self.cv2):
                        cv2.imwrite(  "./11.jpg", gen_image[0])
                ...
                for f in range(12):
                    for k in range(10):
                        ...
                        zero[f] = 1
                        zero[12+k] = 1
                        word_test.append(zero)
                for f in range(5):
                    word_test.append(np.zeros(22))
                for f in range(5):
                    batch_z   = ut.genNoise(test_batch_size,mode=self.noise_mode,ball=self.ball)
                    batch_z = np.concatenate((batch_z, word_test[25*f:25*f+25]), axis=1)
                    gen_image = sess.run(self.gen_image,feed_dict ={noise:batch_z})
                    gen_image = np.array(gen_image)
                    gen_image = gen_image[:,:,:,::-1]          #to rgb
                    all_image.append(gen_image)
        ...
        with tf.Session() as sess:
            ...
            for epo in range(0, epoch):
                ...
                for i in range(0, num_steps):
                    ...
                    batch_z   = ut.genNoise(self.batch_size,mode=self.noise_mode,
                                            ball=self.ball)
                    random_tag = ut.random_tags(self.batch_size)
                    batch_z = np.concatenate((batch_z, random_tag), axis=1)
                    total_dl_fake = 0 
                    total_dl_real = 0
                    total_dl_rft  = 0
                    total_gl      = 0
                    ...
                    for j in range(self.d_update_num):
                        ...
                        _, dl_fake, dl_real, dl_rft = sess.run([self.d_opt, self.d_loss_fake, 
                                                                self.d_loss_real, self.d_loss_rft],
                                              feed_dict = {noise      : batch_z,
                                                           real_image : images,
                                                           real_tags  : words,
                                                           random_tags: random_tag
                                              })
