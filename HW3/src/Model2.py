...
import argparse
class DCGAN():
    def __init__(self, args, noise_dim=100, batch_size=128, lr=0.0002, beta1=0.5):
    ...
    def build_model(self,z,real_image):
        ...
        self.d_opt = tf.train.AdamOptimizer(self.lr, beta1=self.beta1).minimize(d_loss, var_list=d_vars)#only update the discriminator's parameters
        self.g_opt = tf.train.AdamOptimizer(self.lr, beta1=self.beta1).minimize(self.g_loss, var_list=g_vars)#only update the generator's parameters
        ...
        with tf.Session() as sess:
            ...
            for epo in range(0, epoch):
                if (epo % 1 == 0):
                    ...
                    idx = np.random.randint(0,self.batch_size-4)
                    gen_image = np.array(gen_image[ idx : idx+5])
                    for i in range(5):
                        #記得在生產圖片的地方換成  cv.imwrite("../genImage/DCGAN2_80/" + self.name + "_" + str(epo)
                                #               + "_" + str(i) + ".jpg", gen_image[i])
                        skimage.io.imsave("../genImage/DCGAN2_80/" + self.name + "_" + str(epo)#不要
                                                + "_" + str(i) + ".jpg", gen_image[i])
                if (epo != 0):
...
parser.add_argument('--save_path', type=str, default="../models2/")
args = parser.parse_args()
model = DCGAN()
model.train(load=True,epoch=50,args)
exit()
