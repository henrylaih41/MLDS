...
import scipy
import numpy as np
from agent_dir.util import conv2d, DNN, get_real_reward, get_weights
...
class Agent_PG(Agent):
    ...
    def build_model(self):
        ...
        ### Sharing layer
        D1 = DNN(self.diff,[6400,200],0)
        D2 = DNN(D1,[200,100],1)
        V0 = DNN(D2,[100,100],3)
        ### 
        V = DNN(V0,[100,1],2,False)
        D3 = DNN(D2,[100,2],4)
        self.action_prob = D3
        ...
        v_loss = tf.losses.mean_squared_error(labels=self.rewards,predictions=self.V_s)
        a_loss = tf.losses.log_loss(labels=tf.one_hot(self.actions,2),
        ...
        self.actor_opt = tf.train.AdamOptimizer(self.lr).minimize(a_loss)
        self.critic_opt = tf.train.AdamOptimizer(self.lr).minimize(v_loss)
...
###             
                ...
                a_l = np.array(a_l)
                ### update critic V
                _ = sess.run([self.critic_opt], 
                             feed_dict = {
                                 self.diff : s_l,
                                 self.rewards : r_l
                             })
                state_Er = sess.run([self.V_s],
                                   feed_dict = {
                                       self.diff:s_l
                                   })
                w_l = get_weights(state_Er,r_l[-1])
                ### update actor
                _ = sess.run([self.actor_opt], 
                             feed_dict = {
                                 self.diff : s_l,
                                 self.actions : a_l,
                                 self.weights : w_l
            ...
            np.save(self.save_path + "his_r" + ".npy",his_r)
