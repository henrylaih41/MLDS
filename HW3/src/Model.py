...
class DCGAN():
        ...
        def build_generator():
            ...
            with tf.variable_scope("generator") as scope:
                ...
                gen_image = ut.deconv2d(deconv3,[bs,d_w*2,d_h*2,self.image_depth],
                                        f_h=5,f_w=5,name="deconv4",relu=False)
        ...
        def build_discriminator(image,reuse=False):
            ...
            with tf.variable_scope("discriminator") as scope:
                ...
                B2 = tf.get_variable("B2",[1],tf.float32,
                                     tf.truncated_normal_initializer(stddev=0.02))
                #W3 = tf.get_variable("W3",[flatten.get_shape().as_list()[-1]/int(2),1],tf.float32,
                #                     tf.truncated_normal_initializer(stddev=0.02))
                #B3 = tf.get_variable("B3",[1],tf.float32,
                #                     tf.truncated_normal_initializer(stddev=0.02))
                #s_1 = tf.matmul(flatten,W2) + B2
                #s_1 = tf.nn.leaky_relu(s_1)
                #score = tf.matmul(s_1,W3) + B3
    ...
    def train(self,epoch=100,load=False):
        ...
        datasetTrain = Dataset(self.label_dir,self.batch_size,mode="crop")
        ...
        with tf.Session() as sess:
            ...
            for epo in range(0, epoch):
                if (epo % 1 == 0):
                    ...
                    for i in range(5):
                        skimage.io.imsave("../genImage/DCGAN1/" + self.name + "_" + str(epo)
...
model = DCGAN()
model.train(load=True,epoch=80)
exit()
